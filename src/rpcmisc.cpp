...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total pop balance of the wallet\n"
            "  \"privatesend_balance\": xxxxxx, (numeric) the anonymized pop balance of the wallet\n"
...
UniValue mnsync(const UniValue& params, bool fHelp)
{
    ...
    if(strMode == "status") {
        ...
        objStatus.push_back(Pair("AssetID", popnodeSync.GetAssetID()));
        objStatus.push_back(Pair("AssetName", popnodeSync.GetAssetName()));
        objStatus.push_back(Pair("Attempt", popnodeSync.GetAttempt()));
        objStatus.push_back(Pair("IsBlockchainSynced", popnodeSync.IsBlockchainSynced()));
        objStatus.push_back(Pair("IsPopnodeListSynced", popnodeSync.IsPopnodeListSynced()));
        objStatus.push_back(Pair("IsSynced", popnodeSync.IsSynced()));
        objStatus.push_back(Pair("IsFailed", popnodeSync.IsFailed()));
        return objStatus;
    }

    if(strMode == "next")
    {
        popnodeSync.SwitchToNextAsset();
        return "sync updated to " + popnodeSync.GetAssetName();
    }
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"popaddress\"\n"
            "\nReturn information about the given pop address.\n"
            "\nArguments:\n"
            "1. \"popaddress\"     (string, required) The pop address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"popaddress\", (string) The pop address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: Pop address and we have full public key:
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are pop addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) pop address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"popaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"popaddress\"  (string, required) The pop address to use for the signature.\n"
