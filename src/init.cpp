...
void PrepareShutdown()
{
    ...
    CFlatDB<CPopnodeMan> flatdb1("mncache.dat", "magicPopnodeCache");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageGroup(_("Popnode options:"));
    strUsage += HelpMessageOpt("-popnode=<n>", strprintf(_("Enable the client to act as a popnode (0-1, default: %u)"), 0));
    strUsage += HelpMessageOpt("-mnconf=<file>", strprintf(_("Specify popnode configuration file (default: %s)"), "popnode.conf"));
    strUsage += HelpMessageOpt("-mnconflock=<n>", strprintf(_("Lock popnodes from popnode configuration file (default: %u)"), 1));
    strUsage += HelpMessageOpt("-popnodeprivkey=<n>", _("Set the popnode private key"));
    ...
    strUsage += HelpMessageOpt("-privatesendrounds=<n>", strprintf(_("Use N separate popnodes for each denominated input to mix funds (2-16, default: %u)"), DEFAULT_PRIVATESEND_ROUNDS));
...
{
    return FormatParagraph(strprintf(_("Copyright © 2017-%i Pop Foundation Ltd."), COPYRIGHT_YEAR)) + "\n" +
           "\n";
}
...
/** Sanity checks
 *  Ensure that Pop Core is running in a usable environment with all
...
void InitParameterInteraction()
{
    ...
    if (GetBoolArg("-popnode", false)) {
        // popnodes must accept connections from outside
        if (SoftSetBoolArg("-listen", true))
            LogPrintf("%s: parameter interaction: -popnode=1 -> setting -listen=1\n", __func__);
    }
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Pop Core version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
}
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Pop Core is shutting down."));
...
#endif
    // Make sure only a single Pop Core process is using the data directory.
    ...
    try {
        ...
        if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
            return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Pop Core is probably already running."), strDataDir));
    } catch(const boost::interprocess::interprocess_exception& e) {
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Pop Core is probably already running.") + " %s.", strDataDir, e.what()));
    }
    ...
    if (mapArgs.count("-whitelist")) {
        BOOST_FOREACH(const std::string& net, mapMultiArgs["-whitelist"]) {
            ...
            CNode::AddWhitelistedRange(supop);
        }
    }
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Pop Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Pop Core to complete") << "\n";
    ...
    fPopNode = GetBoolArg("-popnode", false);

    if((fPopNode || popnodeConfig.getCount() > -1) && fTxIndex == false) {
        return InitError("Enabling Popnode support requires turning on transaction indexing."
    ...
    if(fPopNode) {
        ...
        if(!GetArg("-popnodeaddr", "").empty()) {
            // Hot popnode (either local or remote) should get its address in
            // CActivePopnode::ManageState() automatically and no longer relies on popnodeaddr.
            return InitError(_("popnodeaddr option is deprecated. Please use popnode.conf to manage your remote popnodes."));
        }

        std::string strPopNodePrivKey = GetArg("-popnodeprivkey", "");
        if(!strPopNodePrivKey.empty()) {
            if(!darkSendSigner.GetKeysFromSecret(strPopNodePrivKey, activePopnode.keyPopnode, activePopnode.pubKeyPopnode))
                return InitError(_("Invalid popnodeprivkey. Please see documenation."));

            LogPrintf("  pubKeyPopnode: %s\n", CBitcoinAddress(activePopnode.pubKeyPopnode.GetID()).ToString());
        } else {
            return InitError(_("You must specify a popnodeprivkey in the configuration. Please see documentation for help."));
        }
    }
    ...
    //lite mode disables all Popnode and Darksend related functionality
    fLiteMode = GetBoolArg("-litemode", false);
    if(fPopNode && fLiteMode){
        return InitError("You can not start a popnode in litemode");
    }
    ...
    uiInterface.InitMessage(_("Loading popnode cache..."));
    CFlatDB<CPopnodeMan> flatdb1("mncache.dat", "magicPopnodeCache");
    if(!flatdb1.Load(mnodeman)) {
        return InitError("Failed to load popnode cache from mncache.dat");
    }

    if(!mnodeman.size()) {
		uiInterface.InitMessage(_("Popnode cache is empty, skipping payments cache..."));
    }
    ...
    // ********************************************************* Step 11c: update block tip in Pop modules

    // force UpdatedBlockTip to initialize pCurrentBlockIndex for DS
    // but don't call it directly to prevent triggering of other listeners like zmq etc.
    // GetMainSignals().UpdatedBlockTip(chainActive.Tip());
    mnodeman.UpdatedBlockTip(chainActive.Tip());
    darkSendPool.UpdatedBlockTip(chainActive.Tip());
    popnodeSync.UpdatedBlockTip(chainActive.Tip());

    // ********************************************************* Step 11d: start pop-privatesend thread
