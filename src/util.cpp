...
const char * const BITCOIN_CONF_FILENAME = "pop.conf";
const char * const BITCOIN_PID_FILENAME = "popd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "pop" is a composite category enabling all Pop-related debug output
            if(ptrCategory->count(string("pop"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\PopCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\PopCore
    // Mac: ~/Library/Application Support/PopCore
    // Unix: ~/.popcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "PopCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/PopCore";
#else
    // Unix
    return pathRet / ".popcore";
#endif
#endif
}
...
boost::filesystem::path GetPopnodeConfigFile()
{
    boost::filesystem::path pathConfigFile(GetArg("-mnconf", "popnode.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty pop.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override pop.conf
